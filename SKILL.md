---
name: loveav
description: "Use this skill as a local-first LoveAV content workbench for Telegram exports, pasted text, curated libraries, v0.5.13-compatible filters, Whos.tv solved-answer collection, and the explicit read-only Svip PikPak tgctl workflow."
---
...
- **Svip PikPak (explicit, read-only):** only for requests that clearly ask for Svip official/admin PikPak replies. Use the existing local `tgctl` reader plus the private `bindings.Svip.chat_id`; fetch structured message history, then classify locally with `scripts/filter_svip_resource_replies.py`. Never ask for Telegram credentials, never mark read, never send/forward/download media, and never access a PikPak URL.

Direct Telegram personal API credential handling, Bot API, Work/cloud execution, background synchronization, remote Raindrop writes, and 123AV account actions are intentionally out of the active scope. The only enabled Telegram-adapter exception is the explicit Svip PikPak workflow above, which delegates bounded read-only history to an already configured local `tgctl`; LoveAV itself never reads or manages API Hash, phone, OTP, 2FA, Session contents, or proxy credentials. The v0.5.13 compatibility document records other adapters for reference only; never propose them or request their credentials in this Skill's default workflow.
...
Manual mode is the default even if a connected adapter exists. Never switch modes merely because a file looks like a Telegram export. The Svip PikPak mode is activated only by an explicit Svip/PikPak request.

The first four content functions (MissAV, Twitter, Bad.news, and Haijiao) share one input contract: any combination of Telegram Desktop HTML/JSON, TXT, CSV, MD, LOG, or pasted plain text. The file type is only an input container; the selected tool decides what can be extracted from its text. Whos.tv has its separate returned-JSON workflow. The Svip PikPak workflow has its own structured tgctl contract in [Svip official PikPak replies](references/svip-pikpak-replies.md).
...
## Svip official PikPak resource replies

Treat these requests as a dedicated deterministic workflow, not as a learned rule and not as an expansion of TG Exporter sender semantics. Read [Svip official PikPak replies](references/svip-pikpak-replies.md) before acting.

- Bind only the exact `chat_id` stored locally at `loveav-data/svip-resource-replies.json` under `bindings.Svip.chat_id`. Never commit that real ID to GitHub.
- For “读取Svip最近1000条官方PikPak回复”, run `python scripts/filter_svip_resource_replies.py --run-tgctl --limit 1000 --pretty`. This reads the latest 1000 structured history messages and filters locally.
- For “提取Svip本月管理员资源链接”, calculate the current local month `[since, until)` and run the same script with `--limit 5000 --since ... --until ... --classification verified_moderator`.
- For “复核Svip可疑PikPak回复”, run with `--view review`.
- The tgctl invocation must be `messages history`; do not add `--sender-role`, `--url-domain`, `send`, `forward`, or `media download`.
- Match only hostname `mypikpak.com` or a true subdomain after URL parsing. `mypikpak.com.evil.com` must never match. Do not access links.
- Primary results are exactly `verified_moderator` plus `trusted_official_reply`; `needs_review` is separate; known ordinary members are excluded and counted separately.
- Never infer a specific administrator from nickname, body signature, link contents, or `forward_origin`; never fabricate `sender_id`.
- The expected real-world counts around 101 primary / 2 review / 1 known-member exclusion are acceptance observations, not hard-coded assertions. Classification evidence is the source of truth.
...
svip.history.read(exact_chat_binding, limit, since, until)
svip.pikpak.classify(structured_messages)
svip.pikpak.primary / svip.pikpak.review
...
- Svip PikPak: each record includes at least `message_id`, `date`, `pikpak_url`, `classification`, and `evidence`; default response separates `primary` and `needs_review`, and separately reports `excluded_known_member_count`.
...
The Svip PikPak workflow is read-only and needs no extra confirmation after the user explicitly requests it. It may read structured history through the existing local tgctl session, but must never read credentials/Session contents, mark messages read, send/forward, download media, mutate groups, or access extracted URLs.
...
- [Svip official PikPak replies](references/svip-pikpak-replies.md) — exact chat binding, local URL matching, deterministic sender classification, and output contract.
